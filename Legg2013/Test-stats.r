...
model <- "Mk"
dataset <- "Legg2013.nex"
analysis_name <- "pps_morpho_arthropods_Legg"
...
for (i in 1:length(sim_reps)){
  ...
  par(mfrow=c(2,2))
  hist(TestStats[,"Consistency Index"], col = "gray74", border = "white", xlim = c(0.16,0.23)
       , xlab = "Consistency Index", main="")
  abline(v=empci, col ="forest green", lty=2, lwd =2)
  abline(v=median(TestStats[,"Consistency Index"]), col ="deepskyblue3", lty=2, lwd =2)
  
  
  hist(TestStats[,"Retention Index"],  col = "gray74", border = "white", xlim = c(0.77,0.81)
       , xlab = "Retention Index", main ="")
  abline(v=empri, col ="forest green", lty=2 ,lwd =2)
  abline(v=median(TestStats[,"Retention Index"]), col ="deepskyblue3", lty=2, lwd =2)
  
    
#  hist(TestStats[,"Consistency Index"],
#       col = "gray60", border = "black",
#       breaks = 20,  # 降低 bin 数量
#       xlim = c(0,1), xlab = "Consistency Index", main = "")
#  
#  hist_data <- hist(TestStats[,"Consistency Index"], plot = FALSE)
#  print(hist_data)
